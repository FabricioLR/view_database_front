...
export const loadRequest = (payload: { payload: Pick<Payload["payload"], "url"|"language"|"setLoad"> }) => action(DatabaseTypes.LOAD_REQUEST, { payload })
...
export const filter = (payload: { payload: Pick<Payload["payload"], "tableName"|"column"|"value"|"tableIndex"> }) => action(DatabaseTypes.FILTER, { payload })

export const saveNewRow = (payload: { payload: Pick<Payload["payload"], "tableName"|"update"|"column"|"value"|"setNewRow"|"tableIndex"> }) => action(DatabaseTypes.SAVE_NEW_ROW, { payload })
...
export const deleteRow = (payload: { payload: Pick<Payload["payload"], "row"|"tableName"|"tableIndex"> }) => action(DatabaseTypes.DELETE_ROW, { payload })
...
export const updateValue = (payload: { payload: Pick<Payload["payload"], "tableName"|"row"|"update"|"tableIndex"|"rowIndex"> }) => action(DatabaseTypes.UPDATE_VALUE, { payload })
